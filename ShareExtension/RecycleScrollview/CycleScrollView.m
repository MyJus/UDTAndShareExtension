...
- (void)refreshScrollView {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        dispatch_async(dispatch_get_global_queue(0, 0), ^{
            UIImage *showImage = [UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:[self->curImages objectAtIndex:i]]]];//[curImages objectAtIndex:i];
            dispatch_async(dispatch_get_main_queue(), ^{
                imageView.image = [self scaleImage:showImage];
            });
        });
